...
H~0~: **[pH does not predict abalone growth]**  

H~A~: **[pH does predict abalone growth]**
...
# creating object called abalone_clean to store cleaned abalone data
abalone_clean <- abalone  |>
  # clean_names function takes away spaces and makes everything lowercase
  clean_names() |>  
...
ggplot(data = abalone_clean, # using abalone_clean data 
       aes(x = mean_ph, # x-axis is mean pH
           y = change_in_area)) + # y-axis is change in shell surface area
  # first layer is geom_point
  geom_point(size = 4,    # changing size of points
             stroke = 1, 
             fill = "firebrick4", # filling points with firebrick4 color
             shape = 21) # changing shape of points
```

Where does the missing value come from?  

**The missing value comes from a pair of tanks in which the pH sensor malfunctioned, so there is no pH reading for that tank.**
...
abalone_model <- lm(
  change_in_area ~ mean_ph, # formula: change in shell SA as a function of mean pH
  data = abalone_clean      # using abalone_clean data frame
)
```
...
plot(abalone_model)    # will display diagnostic plots 
...
'x', 'predicted', 'std.error', 'conf.low', 'conf.high', 'group' (not going to worry about group column for now) 
...
At pH = 8 the model predicts abalone shell surface area growth of 5.36 [95% CI: 5.08, 5.64]
