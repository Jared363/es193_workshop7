...
# creating sonodora_clean object to store cleaned sonodora data 
...
# base layer: ggplot
ggplot(
      # using sonodora_summary data
      data = sonadora_sum, 
      # x-axis is elevation (meters)
      # y-axis is mean temperature (Celcius)
       aes(x = elevation_m,
           y = mean_temp_c)) +
  ...
  labs(x = "Elevation (M)",
       ...
       title = "Mean Temperature in Relation to Elevation") +
  theme_classic()
...
# creating temperature_model to store model output
temperature_model <- lm(
  mean_temp_c ~ elevation_m, # formula: response ~ predictor
  data = sonadora_sum # using sonodora_sum data frame
  )
...
**Intercept**: 24.78
**Slope**: -0.005545 degrees Celcius/Meter gain of elevation
...
**For each meter of elevation gain, you would expect an approximate decrease in temperature equivalent to 0.01 ± 0.0003 (SE) °C.**
...
# calculate the temperature prediction at elevation = 420
ggpredict(
  temperature_model,           # model object
  terms = "elevation_m[420]"   # predictor (in quotation marks) and predictor value in brackets
)
```

**At 420 m, the predicted temperature is 22.45 °C (95% CI: [22.28, 22.62]).**
